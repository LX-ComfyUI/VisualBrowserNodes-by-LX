                ...
                bg.querySelector("#ckpt-add-local-img-btn").onclick = () => {
                    ...
                    fileInput.onchange = async () => {
                        ...
                        try {
                            ...
                            const resp = await api.fetchApi("/visual_checkpoint/save_local_image", { method: "POST", body: fd });
                            const result = await resp.json();
                            if (result.status === "ok") {
                                if (!window.comfyVisualCheckpointCache[selectedFilename]) window.comfyVisualCheckpointCache[selectedFilename] = {};
                                window.comfyVisualCheckpointCache[selectedFilename].customCover = result.url;
